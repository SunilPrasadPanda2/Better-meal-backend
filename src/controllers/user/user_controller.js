...
import googleVerify from "../../utils/googleSignup.js";
...
const googleSignUp = async (req, res) => {
  ...
  let payload;
  try {
    payload = await googleVerify(res, value.idToken);
    if (!payload) {
      return ApiResponse(res, 401, "Invalid Google token");
    }
  } catch (error) {
    return ApiResponse(res, 500, "Internal Server Error for Google Verify");
  }
  ...
  try {
    ...
    if (!user) {
      try {
        ...
        if (userCreated) {
          ...
          const refreshToken = userCreated.generateAuthRefreshToken();
          res.status(200).json({
          ...
          res
            .status(500)
            .json({ message: "Internal Server Error for created user" });
        }
      } catch (error) {
        return ApiResponse(
          res,
          500,
          "Internal Server Error for user creation",
          error
        );
      }
    }
    ...
    return ApiResponse(
      res,
      500,
      "Internal Server Error for user present",
      error
    );
  }
};
...
const sendOtp = async (req, res) => {
  ...
  if (!phonePattern.test(req.body.phone)) {
    return res
      .status(400)
      .json({ message: "Please enter 10 digits for mobile number" });
  }

  try {
    const isPhoneExists = await phoneVerification.findOne({
    ...
    if (isPhoneExists) {
      ...
      return res
        .status(201)
        .json({ message: "OTP generated successfully", phone });
    }
  } catch (e) {
    return res.status(500).json({
      message: "OTP couldn't be generated. Something went wrong",
      error: e.message,
    });
  }
};
...
const signup = async (req, res) => {
  ...
  if (isUserExists) {
    return res
      .status(409)
      .json({ message: "User email or phone already exists" });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "User created successfully", user: customer });
  } catch (err) {
...
const forgotPassword = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "A link has been sent to your mail Id" });
};
...
const updateProfile = async (req, res) => {
  ...
  try {
    ...
    if (profileUpdate.modifiedCount === 0) {
      return res
        .status(404)
        .json({ message: "No profile was updated. User not found." });
    }
    const something = gut.gutCleansingRecommendation(userId, res);
    return res
      .status(200)
      .json({ message: "Profile updated successfully", profile: updatedData });
  } catch (e) {
    return res
      .status(500)
      .json({ message: "Failed to update profile due to an internal error" });
  }
};
...
const changePassword = async (req, res) => {
  ...
  if (error)
    return ApiResponse(res, 400, "Validation failed", {
    ...
    if (passwordMatch) {
      ...
      if (userUpdatePassword.modifiedCount > 0) {
        ...
        return ApiResponse(
          res,
          500,
          "Something went wrong password could not be updated"
        );
      }
    } else {
      return ApiResponse(
...
const getDashboardScore = async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select(
      "nutritionscore smoker drinker gutscore"
    );
    if (user) return ApiResponse(res, 200, "user details", user);
    else return ApiResponse(res, 500, "Something went wrong");
  } catch (error) {
...
const exploreSection = async (req, res) => {
  ...
  if (error) return ApiResponse(res, 400, "Validation Error", error);

  if (!req.user._id) return ApiResponse(res, 401, "User not authenticated");

  try {
    const date = new Date(value.date);
    const explore = await ExploreMeal.findOne({ date: date }).populate("tags");
    if (explore) return ApiResponse(res, 200, "Explore Section", explore);
