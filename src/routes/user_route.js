...
router.route("/google-signin").post(user.googleSignUp);

router.route("/send-otp").post(user.sendOtp);
router.route("/phone-verification").post(user.phoneVerificationOtp);
router.route("/signup").post(user.signup);
router.route("/google-signup").post(user.googleSignUp);
router.route("/login").post(user.login);
router.route("/forgot-password").post(user.forgotPassword);
router
  .route("/profile-update")
  .post(auth, upload.single("image"), user.updateProfile); //if profile is updated then the gut cleanse recommendation will be added.
...
router
  .route("/get-consumed-meal-for-today")
  .post(auth, meal.getConsumedTodaysMeal);
...
router
  .route("/remove-favourite-meal")
  .post(auth, favouriteMeals.removeFavouriteMeal);
...
router
  .route("/disease-score-calculation")
  .post(auth, disease.answerDiseaseQuestions);

router.route("/add-gut-cleanse").post(auth, gut.gutCleanse);
router.route("/deleteAcc/:_id").get(auth, user.deleteUser);
